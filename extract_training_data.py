...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        for index in range(1, 4):
            # if the index exists, it means that there is some token there that needs to be evaluated
            if index <= stack_length:
                position_in_sentence = stack[-1 * index]


                pos_of_word = pos[position_in_sentence]
                word = str(words[position_in_sentence])
                ...
                if pos_of_word == '<CD>':
                    returner.append(0)
                # NNP
                elif pos_of_word == '<NNP>':
                    returner.append(1)
                # Root
                elif pos_of_word is None:
                    returner.append(3)

                else:
                    # normal word
                    try:
                        returner.append(self.word_vocab[word])
                    # UNK
                    except KeyError:
                        returner.append(2)
            ...
            # if there is no token, it means that the list was not long enough and the rest of the position should be
            # filled with Null values which is 4
            else:
                returner.append(4)
        ...
        for index in range(1, 4):
            ...
            if index <= buffer_length:
                ...
                position_in_sentence = buffer[-1 * index]
                pos_of_word = pos[position_in_sentence]
                word = words[position_in_sentence]
        ...
        return np.array(returner).reshape(1, 6)

    def get_output_representation(self, output_pair):

        # Should I do this using the other method?? How does that thing work.
